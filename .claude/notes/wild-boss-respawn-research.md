...
### bug 4(2026-07-22 使用者實測發現):捕捉後倒數顯示 22 小時,跟這次死亡完全無關的舊數字
用使用者測試機的 agent token 直接拉 `boss-respawns` 原始資料驗證:`1_10_plain_F_Boss_FairyDragon`
的 `respawnedAt`(01:47:51)只比 `diedAt`(01:48:21,即這次捕捉時間)早 30 秒,但 `respawnInterval`
卻是 79189 秒(≈22 小時)——跟這兩個時間點完全對不上,證明是**很久以前某一輪的舊測量值殘留**。

根因:「觀測有斷過、這次測到的間隔不可信」的分支(main.lua 187-197 行)只是不覆蓋 `respawnInterval`
新值,沒有把舊值清掉——於是舊數字被之後**每一次**死亡繼續沿用,顯示一個跟本次死亡毫無關係的倒數。
修法:進這個分支時把 `t.respawnInterval` 明確設回 `-1`(v1.4→v1.5)。log 訊息也補上
"cleared stale value" 方便辨識。此修法只防止未來繼續累積殘留;**已經寫進目前 boss-state.json 的
79189 這個壞值不會被追溯清除**,要等這隻頭目再走一輪死亡→復活才會被新邏輯覆蓋或正確清空。
