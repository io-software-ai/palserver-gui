...
export function registerRoutes(
  ...
  const MessageBridgePatchSchema = z.object({
    ...
    discord: z.object({ ...MessageBridgeRulePatchShape, added: z.boolean().optional(), enabled: z.boolean().optional(), channelId: z.string().trim().max(100).optional(), adminIds: z.array(z.string().trim().min(1).max(128)).max(50).optional(), language: MessageBridgeLanguageSchema.optional(), proxyEnabled: z.boolean().optional(), proxyUrl: z.string().trim().max(1000).optional(), token: z.string().max(2000).optional() }).optional(),
  ...
  app.post("/api/instances/:id/message-bridge/webhook", async (req, reply) => {
    ...
    try {
      await messageBridge.receiveWebhook(rec.id, supplied, body.userId ?? "", body.author ?? "Webhook", body.text);
      return { ok: true };
    } catch (err) {
