...
function readIvs(p: Record<string, unknown>): PdPal["ivs"] | undefined {
  const obj = p.IVs;
  ...
  const hp = readNumber(p.Talent_HP ?? p.TalentHP);
  const atk = readNumber(p.Talent_Shot ?? p.TalentAttack ?? p.Talent_Melee);
